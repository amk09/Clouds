...
// Taken from https://iquilezles.org/articles/distfunctions
//scalar field where the valuea at an point indicates the distance to the nearest surface. 
...
{    
    //If the intersectionT is > epsilon (0.0001) & intersectionT is smaller than large number/ last updated T. Normal becomes intersection normal,
    //materialID is Checker_Floor_material id or point light id. And t is updated into interection t. 
...
//find the cloest intersect (light burb or the plane)
float IntersectOpaqueScene(in vec3 rayOrigin, in vec3 rayDirection, out int materialID, out vec3 normal)
{
    ...
    for(int lightIndex = 0; lightIndex < NUM_LIGHTS; lightIndex++)
    {
        //update the intersectionT, and materialID, and intersectionNormal.
        UpdateIfIntersected(
            t,
...
{    
    float iTime = 0;
    glm::vec3 fbmCoord = (pos + 2.0 * vec3(iTime, 0.0, iTime)) / 1.5f;
...
vec3 Diffuse(glm::vec3 normal, glm::vec3 lightVec, glm::vec3 diffuse)
{
...
//color blend interportation
float Luminance(vec3 color)
{
    return (color.r * 0.3) + (color.g * 0.59) + (color.b * 0.11);
}
//if the lumiance of color is < 0.009
bool IsColorInsignificant(vec3 color)
{
    const float minValue = 0.009;
    return Luminance(color) < minValue;
}
//Calculate the ligting for all the lights. need material, color of the nowest pixel. 
...
glm::vec3 Render( glm::vec3 rayOrigin, glm::vec3 rayDirection)
{
    float depth = LARGE_NUMBER;
    glm::vec3 opaqueColor = vec3(0.0f);
    
    glm::vec3 normal;
    float t;
    int materialID = INVALID_MATERIAL_ID;
    //find the cloest intersect distance with updated materialID(which object it intersects with and its corresponding normal)
    t = IntersectOpaqueScene(rayOrigin, rayDirection, materialID, normal);
    if( materialID != INVALID_MATERIAL_ID )
    {
        //IF it doesn't intersect anything, do raymarching.
    ...
    //how far you have gone inside the depth
    float volumeDepth = IntersectVolumetric(rayOrigin, rayDirection, depth);
    float opaqueVisiblity = 1.0f;
    vec3 volumetricColor = vec3(0.0f);//all black

    if(volumeDepth > 0.0)
    {   //grey color
        const glm::vec3 volumeAlbedo = vec3(0.8);
